...
function initSpeechRecognition() {
    ...
    recognition.onend = () => {
        starting = false;
        // Do not auto-restart to avoid loops; user must restart manually
        isListening = false;
...
// Detectar prefijo de API según entorno (raíz o subcarpeta)
function getApiPrefix() {
    if (window.location.pathname.startsWith('/fabiantalkenglish')) {
        return '/fabiantalkenglish/api';
    }
    return '/api';
}
...
// Obtener respuesta de la IA usando Groq (Llama 3)
async function getAIResponse(userMessage) {
    try {
        ...
        // Llamar al backend usando el prefijo correcto
        const response = await fetchWithTimeout(getApiPrefix() + '/chat', {
            method: 'POST',
            headers: {
...
// Traducir texto usando la API del backend
async function translateText(text) {
    try {
        const response = await fetchWithTimeout(getApiPrefix() + '/translate', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function startListening() {
    ...
    try {
        // Request microphone permission explicitly to ensure prompt appears
